...
interface Hobby {
  id: string; title: string; description: string | null; icon: string | null; image_url: string | null; sort_order: number;
}
...
const ICON_OPTIONS = [
  "heart", "music", "camera", "gaming", "reading", "art",
  "fitness", "travel", "coffee", "cooking", "hiking", "globe",
];
...
export default function AdminHobbiesPage() {
  ...
  async function load() {
    const { data } = await createClient().from("hobbies_interests").select("*").order("sort_order");
    setItems(data ?? []);
  }
  useEffect(() => { load(); }, []);
  ...
  function move(index: number, dir: -1 | 1) {
    ...
    start(async () => { await adminReorder("hobbies_interests", arr.map((item, i) => ({ id: item.id, sort_order: i })), "/admin/hobbies"); });
  }
  ...
  return (
    <AdminShell title="Hobbies & Interests" description={`${items.length} hobbies`}
      ...
      <div className="max-w-2xl space-y-2">
        ...
        {items.length === 0 && <p className="py-8 text-center text-muted-foreground">No hobbies yet.</p>}
      </div>
      ...
      <FormModal title={modal.item?.id ? "Edit Hobby" : "New Hobby"} open={modal.open} onClose={close}>
        <div className="space-y-4">
          ...
          <div>
            ...
            <select value={modal.item?.icon ?? "heart"} onChange={(e) => setField("icon", e.target.value)} className={inputCls}>
              {ICON_OPTIONS.map((ic) => <option key={ic} value={ic}>{ic.charAt(0).toUpperCase() + ic.slice(1)}</option>)}
            </select>
          </div>
